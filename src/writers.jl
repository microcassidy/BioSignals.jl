...
function write_binary(io::IO,header::Header,samples::Vector{Int32},::WfdbFormat{format310})
...
# function read_binary(io::IOStream,header::Header,::WfdbFormat{format24})::Vector{Int32}
#   return reinterpret(Int24,read(io))
function write_binary(io::IO,header::Header,samples::Vector{Int32},::WfdbFormat{format311})
...
# Each sample is represented by a 10-bit two’s-complement amplitude. Three samples
# are bit-packed into a 32-bit integer as for format 310, but the layout is
# different. Each set of four bytes is stored in little-endian order (least
# significant byte first, most significant byte last). The first sample is
# obtained from the 10 least significant bits of the 32-bit integer, the second is
# obtained from the next 10 bits, the third from the next 10 bits, and the two
# most significant bits are unused. This process is repeated for each successive
